...
function Write-FontToolsLog([string]$msg) {
...
if (-not (Test-Path $NodeExe)) {
    ...
    Write-FontToolsLog "Extracting Node.js..."
...
if (-not (Test-Path (Join-Path $FontTools "node_modules\lv_font_conv"))) {
    Write-FontToolsLog "Installing lv_font_conv (npm)..."
...
if ($LASTEXITCODE -ne 0) {
    Write-FontToolsLog "Installing Python lz4 for font compression..."
    ...
    Write-FontToolsLog "Python lz4: already installed"
}
Write-FontToolsLog "OK: $LvFontConv"
