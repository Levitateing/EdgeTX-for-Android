...
def _resolution_scale(w: int, h: int) -> float:
    util_dir = SCRIPT_DIR.parent / "radio" / "src" / "targets" / "android" / "util"
    if str(util_dir) not in sys.path:
        sys.path.insert(0, str(util_dir))
    from edge_tx_display_scale import bitmap_scale_factor

    return bitmap_scale_factor(w, h)
...
def scale_dimension(value: int, scale: float, lcd_w: int | None = None, lcd_h: int | None = None) -> int:
    """Scale a design-px dimension; prefer LAYOUT_SCALE integer math when WxH known."""
    if lcd_w is not None and lcd_h is not None:
        try:
            from edge_tx_display_scale import layout_scale_px
            return max(1, layout_scale_px(value, lcd_w, lcd_h))
        except Exception:
            pass
    return round(value * scale)
...
def update_src_list_csv(verbose: bool = False) -> int:
    ...
    Derives dimensions from existing PNGs (prefers 800x480 stock resolution).
    ...
    with Progress(
        ...
        for svg_path in svg_paths:
            ...
            # Try to get dimensions from existing PNGs (prefers 800x480 stock resolution)
            width = ""
            height = ""
...
def resolution_scale(resolution: str) -> Optional[float]:
    """LAYOUT_SCALE bitmap factor for WxH (480-base design)."""
...
def process_resolution(
    ...
    if scale is None:
        ...
        print(f"Supported resolutions: {', '.join(RESOLUTIONS.keys())}, or any WxH with computed scale")
        return 1

    m_res = re.fullmatch(r"(\d+)x(\d+)", resolution)
    lcd_w = int(m_res.group(1)) if m_res else None
    lcd_h = int(m_res.group(2)) if m_res else None
    ...
    with Progress(
        ...
        for row in csv_rows:
            ...
            svg_path = SRC_DIR / f"{file_name}.svg"
            png_path = out_dir / f"{file_name}.png"
            scaled_width = scale_dimension(width, scale, lcd_w, lcd_h)
            scaled_height = scale_dimension(height, scale, lcd_w, lcd_h)
