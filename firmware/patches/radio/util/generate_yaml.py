...
def is_real_cpp_struct(struct):
    if 'anonymous' in struct.name:
        return False
    for prefix in ('struct_unsigned', 'struct_signed', 'struct_string', 'struct_swtch'):
        if struct.name.startswith(prefix):
            return False
    return hasattr(struct, 'var_name') and len(struct.var_name) > 0

def struct_yaml_bits(struct):
    return sum(elmt.bits for elmt in struct.get_elmts())

def get_cpp_type(ast, var_type):
    for s in ast.get_structs():
        if s.name == var_type:
            return s.var_name
    return None

def yaml_struct_field_checks(ast, root_nodes):
    checks = []
    for tn in root_nodes:
        struct_name = 'struct_' + tn
        for s in ast.get_structs():
            if s.name != struct_name:
                continue
            for elmt in s.get_elmts():
                if elmt.type != 'struct':
                    continue
                cpp_type = get_cpp_type(ast, elmt.var_type)
                if cpp_type:
                    checks.append({
                        'parent': tn,
                        'field': elmt.name,
...
template.globals['is_real_cpp_struct'] = is_real_cpp_struct
template.globals['struct_yaml_bits'] = struct_yaml_bits
template.globals['yaml_struct_field_checks'] = lambda: yaml_struct_field_checks(RootAST, top_node_names)
