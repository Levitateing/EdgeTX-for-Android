                                    ...
                                    int screenNum, int zoneNum) const
{
  ...
  // On high-res builds report 800x480-class logical sizes so legacy widgets
  // match TX16-era scripts; lcd.* drawing scales back to physical pixels.
  lua_newtable(lsWidgets);
  l_pushtableint(lsWidgets, "x", 0);
  l_pushtableint(lsWidgets, "y", 0);
#if LUA_SCRIPT_LCD_COMPAT
  l_pushtableint(lsWidgets, "w", luaScriptCompatToLogical(rect.w));
  l_pushtableint(lsWidgets, "h", luaScriptCompatToLogical(rect.h));
  l_pushtableint(lsWidgets, "xabs", luaScriptCompatToLogical(rect.x));
  l_pushtableint(lsWidgets, "yabs", luaScriptCompatToLogical(rect.y));
#else
