...
#include "lua/lua_script_lcd.h"
#include "lua/lua_script_lcd_compat.h"
...
static inline bool luaScaleOn() { return luaScriptCompatScaleEnabled; }
static inline coord_t SX(coord_t x) { return luaScaleOn() ? luaScriptCompatScaleX(x) : x; }
static inline coord_t SY(coord_t y) { return luaScaleOn() ? luaScriptCompatScaleY(y) : y; }
static inline coord_t SS(coord_t v) { return luaScaleOn() ? luaScriptCompatScaleSize(v) : v; }
static inline coord_t US(coord_t v) { return luaScaleOn() ? luaScriptCompatUnscaleSize(v) : v; }
static inline coord_t UX(coord_t x) { return luaScaleOn() ? luaScriptCompatUnscaleX(x) : x; }
static inline coord_t UY(coord_t y) { return luaScaleOn() ? luaScriptCompatUnscaleY(y) : y; }
#else
static inline coord_t SX(coord_t x) { return x; }
static inline coord_t SY(coord_t y) { return y; }
static inline coord_t SS(coord_t v) { return v; }
static inline coord_t US(coord_t v) { return v; }
static inline coord_t UX(coord_t x) { return x; }
static inline coord_t UY(coord_t y) { return y; }
#endif

static coord_t luaLcdActiveW()
{
#if LUA_SCRIPT_LCD_COMPAT
  if (luaScaleOn()) return luaScriptCompatLcdW;
#endif
  return luaLcdBuffer ? luaLcdBuffer->width() : LCD_W;
}

static coord_t luaLcdActiveH()
{
#if LUA_SCRIPT_LCD_COMPAT
  if (luaScaleOn()) return luaScriptCompatLcdH;
#endif
  return luaLcdBuffer ? luaLcdBuffer->height() : LCD_H;
}

static coord_t luaActiveFontHeight(LcdFlags flags)
{
  return US(getFontHeight(flags));
}

static int luaActiveTextWidth(const char* s, int len, LcdFlags flags)
{
  return (int)US(getTextWidth(s, len, flags));
}
...
static int luaLcdDrawPoint(lua_State *L)
{
  ...
  luaLcdBuffer->drawPixel(SX(x), SY(y), COLOR_VAL(flags));
...
static int luaLcdDrawLine(lua_State *L)
{
  ...
  if (x1 > luaLcdActiveW() || y1 > luaLcdActiveH() || x2 > luaLcdActiveW() ||
      y2 > luaLcdActiveH())
    return 0;

  coord_t px1 = SX(x1), py1 = SY(y1), px2 = SX(x2), py2 = SY(y2);
  if (pat == SOLID) {
    if (x1 == x2) {
      luaLcdBuffer->drawVerticalLine(px1, py1 < py2 ? py1 : py2, py1 < py2 ? (py2 - py1) + 1 : (py1 - py2) + 1, pat, flags);
      return 0;
    } else if (y1 == y2) {
      luaLcdBuffer->drawHorizontalLine(px1 < px2 ? px1 : px2, py1, px1 < px2 ? (px2 - px1) + 1 : (px1 - px2) + 1, pat, flags);
      return 0;
    }
  }

  luaLcdBuffer->drawLine(px1, py1, px2, py2, pat, flags);
...
static void drawString(lua_State *L, const char * s, LcdFlags flags)
{
  ...
  if (invers) {
    ...
    int height = luaActiveFontHeight(flags & 0xFFFF) + 2 * INVERT_BOX_MARGIN;
    int width = luaActiveTextWidth(s, 0, flags);
    ...
    luaLcdBuffer->drawSolidFilledRect(SX(ix), SY(y - INVERT_BOX_MARGIN), SS(width), SS(height), color);
  } else {
    if ((flags & BLINK) && !BLINK_ON_PHASE)
      return;
    if (flags & SHADOWED)
      luaLcdBuffer->drawText(SX(x + 1), SY(y + 1), s, flags & 0xFFFF); // force black
    flags = (flags & 0xFFFF) | colorToRGB(flags);
  }
  
  luaLcdBuffer->drawText(SX(x), SY(y), s, flags);
}
...
static int luaLcdSizeText(lua_State *L)
{
  ...
  lua_pushinteger(L, luaActiveTextWidth(s, 0, flags));
  lua_pushinteger(L, luaActiveFontHeight(flags & 0xFFFF));
  return 2;
}
...
static int luaLcdDrawTextLines(lua_State *L)
{
  ...
  if (invers) {
    ...
    // Draw color box
    luaLcdBuffer->drawSolidFilledRect(SX(x), SY(y), SS(w), SS(h), color);
  } else {
    if ((flags & BLINK) && !BLINK_ON_PHASE)
      return 0;
    if (flags & SHADOWED)
      luaLcdBuffer->drawTextLines(SX(x + 1), SY(y + 1), SS(w), SS(h), s, flags & 0xFFFF); // force black
    flags = (flags & 0xFFFF) | colorToRGB(flags);
  }
  
  maxP = luaLcdBuffer->drawTextLines(SX(x), SY(y), SS(w), SS(h), s, flags);
  if (!invers && flags & SHADOWED) {
    maxP.x++;
    maxP.y++;
  }
  lua_pushinteger(L, UX(maxP.x));
  lua_pushinteger(L, UY(maxP.y));
  return 2;
}
...
static int luaLcdDrawChannel(lua_State *L)
{
  ...
  luaLcdBuffer->drawSensorCustomValue(SX(x), SY(y), (channel-MIXSRC_FIRST_TELEM)/3, value, flags);
...
static int luaLcdDrawSwitch(lua_State *L)
{
  ...
  LcdFlags flags = luaL_optinteger(L, 4, 0);
  flags = colorToRGB(flags);
  luaLcdBuffer->drawSwitch(SX(x), SY(y), s, flags);
...
static int luaLcdDrawSource(lua_State *L)
{
  ...
  LcdFlags flags = luaL_optinteger(L, 4, 0);
  flags = colorToRGB(flags);
  luaLcdBuffer->drawSource(SX(x), SY(y), s, flags);
...
#if LUA_SCRIPT_LCD_COMPAT
    if (luaScaleOn()) {
      float s = (scale ? (scale / 100.0f) : 1.0f) * luaScriptCompatScale();
      luaLcdBuffer->drawBitmap(SX(x), SY(y), b, 0, 0, 0, 0, s);
    } else
#endif
    if (scale) {
      luaLcdBuffer->drawBitmap(SX(x), SY(y), b, 0, 0, 0, 0, scale/100.0f);
    }
    else {
      luaLcdBuffer->drawBitmap(SX(x), SY(y), b);
    }
  }
...
static int luaLcdDrawBitmapPattern(lua_State *L)
{
  ...
  if (m) {
    ...
    flags = colorToRGB(flags);
    luaLcdBuffer->drawBitmapPattern(SX(x), SY(y), reinterpret_cast<const MaskBitmap*>(m), flags);
  }
...
static int luaLcdDrawBitmapPatternPie(lua_State *L)
{
  ...
  if (m) {
    ...
    LcdFlags flags = luaL_optinteger(L, 6, 0);
    flags = colorToRGB(flags);
    luaLcdBuffer->drawBitmapPatternPie(SX(x), SY(y), reinterpret_cast<const MaskBitmap*>(m), flags, startAngle, endAngle);
  }
...
static int luaLcdDrawRectangle(lua_State *L)
{
  ...
  luaLcdBuffer->drawRect(SX(x), SY(y), SS(w), SS(h), SS(thickness), SOLID, flags, opacity);
...
static int luaLcdDrawFilledRectangle(lua_State *L)
{
  ...
  luaLcdBuffer->drawFilledRect(SX(x), SY(y), SS(w), SS(h), SOLID, flags, opacity);
...
static int luaLcdInvertRect(lua_State *L)
{
  ...
  luaLcdBuffer->invertRect(SX(x), SY(y), SS(w), SS(h), flags);
...
static int luaLcdDrawGauge(lua_State *L)
{
  ...
  luaLcdBuffer->drawRect(SX(x), SY(y), SS(w), SS(h), SS(1), 0xff, flags);
  uint16_t len = limit((uint16_t)1, uint16_t(w*num/den), uint16_t(w));
  luaLcdBuffer->drawSolidFilledRect(SX(x+1), SY(y+1), SS(len), SS(h-2), flags);
...
static int luaLcdDrawCircle(lua_State *L)
{
  ...
  luaLcdBuffer->drawCircle(SX(x), SY(y), SS(r), flags);
...
static int luaLcdDrawFilledCircle(lua_State *L)
{
  ...
  luaLcdBuffer->drawFilledCircle(SX(x), SY(y), SS(r), flags);
...
static int luaLcdDrawTriangle(lua_State *L)
{
  ...
  luaLcdBuffer->drawLine(SX(x1), SY(y1), SX(x2), SY(y2), SOLID, flags);
  luaLcdBuffer->drawLine(SX(x2), SY(y2), SX(x3), SY(y3), SOLID, flags);
  luaLcdBuffer->drawLine(SX(x3), SY(y3), SX(x1), SY(y1), SOLID, flags);
...
static int luaLcdDrawFilledTriangle(lua_State *L)
{
  ...
  luaLcdBuffer->drawFilledTriangle(SX(x1), SY(y1), SX(x2), SY(y2), SX(x3), SY(y3), flags);
...
static int luaLcdDrawArc(lua_State *L)
{
  ...
  if (r > 0) {
    coord_t pr = SS(r);
    luaLcdBuffer->drawAnnulusSector(SX(x), SY(y), pr > 0 ? pr - 1 : 0, pr, start, end, flags);
  }
...
static int luaLcdDrawPie(lua_State *L)
{
  ...
  if (r > 0)
    luaLcdBuffer->drawAnnulusSector(SX(x), SY(y), 0, SS(r), start, end, flags);
...
static int luaLcdDrawAnnulus(lua_State *L)
{
  ...
  luaLcdBuffer->drawAnnulusSector(SX(x), SY(y), SS(r1), SS(r2), start, end, flags);
...
static int luaLcdDrawLineWithClipping(lua_State *L)
{
  ...
  coord_t pxmin = SX(xmin), pxmax = SX(xmax), pymin = SY(ymin), pymax = SY(ymax);
  if (dc_xmin > pxmin) pxmin = dc_xmin;
  if (dc_xmax < pxmax) pxmax = dc_xmax;
  if (dc_ymin > pymin) pymin = dc_ymin;
  if (dc_ymax < pymax) pymax = dc_ymax;
  
  luaLcdBuffer->setClippingRect(pxmin, pxmax, pymin, pymax);
  luaLcdBuffer->drawLine(SX(x1), SY(y1), SX(x2), SY(y2), pat, flags);
...
static int luaLcdDrawHudRectangle(lua_State *L)
{
  ...
  drawHudRectangle(luaLcdBuffer, pitch, roll, SX(xmin), SX(xmax), SY(ymin), SY(ymax), flags);
