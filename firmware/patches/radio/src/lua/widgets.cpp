...
void luaInitThemesAndWidgets()
{
  ...
  if (lsWidgets) {
    ...
    // Widgets use the same 800x480-class logical coordinates as radio scripts.
    lua_pushinteger(lsWidgets, 800);
    lua_setglobal(lsWidgets, "LCD_W");
    lua_pushinteger(lsWidgets, 480);
    lua_setglobal(lsWidgets, "LCD_H");
#endif
