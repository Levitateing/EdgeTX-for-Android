...
void LuaWidget::updateZoneRect(rect_t rect, bool updateUI)
{
  if (lsWidgets) {
    ...
    if (updateTable("w", luaScriptCompatToLogical(rect.w))) changed = true;
    if (updateTable("h", luaScriptCompatToLogical(rect.h))) changed = true;
    if (updateTable("xabs", luaScriptCompatToLogical(rect.x))) changed = true;
    if (updateTable("yabs", luaScriptCompatToLogical(rect.y))) changed = true;
...
void LuaWidget::refresh(BitmapBuffer* dc)
{
  ...
  // Save tool-script scale state; apply widget zone/fullscreen mapping before
  // touch unscale and lcd.* drawing.
  const bool savedScaleEnabled = luaScriptCompatScaleEnabled;
  const float savedScaleFactor = luaScriptCompatScaleFactor;
  const coord_t savedOffX = luaScriptCompatOffX;
  const coord_t savedOffY = luaScriptCompatOffY;
  const coord_t savedLcdW = luaScriptCompatLcdW;
  const coord_t savedLcdH = luaScriptCompatLcdH;

  if (fullscreen) {
    luaScriptCompatLcdReset();
    luaScriptCompatScaleApplyLetterbox();
  } else {
    luaScriptCompatScaleApplyZone(width(), height());
  }
...
#if LUA_SCRIPT_LCD_COMPAT
  luaScriptCompatScaleEnable(savedScaleEnabled);
  luaScriptCompatScaleFactor = savedScaleFactor;
  luaScriptCompatOffX = savedOffX;
  luaScriptCompatOffY = savedOffY;
  luaScriptCompatLcdW = savedLcdW;
  luaScriptCompatLcdH = savedLcdH;
#endif
  // Remove LCD
  luaLcdAllowed = lla;
