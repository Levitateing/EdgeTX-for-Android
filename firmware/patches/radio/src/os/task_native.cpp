                 ...
                 void* stack, unsigned stack_size, unsigned priority)
{
...
#if defined(__ANDROID__)
  auto nt = std::make_unique<native_thread>();
  pthread_attr_t attr;
  pthread_attr_init(&attr);
  const size_t stack_bytes =
      std::max<size_t>(stack_size * 4, 8 * 1024 * 1024);
  pthread_attr_setstacksize(&attr, stack_bytes);
  const int rc =
      pthread_create(&nt->pt, &attr,
                     +[](void* p) -> void* {
                       _task_stub(static_cast<run_context*>(p));
                       return nullptr;
                     },
                     ctx);
  ...
  h->_thread_handle = std::move(nt);
#else
  h->_thread_handle =
      std::make_unique<std::thread>([=]() { _task_stub(ctx); });
#endif
