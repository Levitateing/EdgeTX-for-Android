...
{
  if (_sdl_audio_device == 0) return 0;
  // S16 mono: 2 bytes/sample
  const uint32_t bytes = SDL_GetQueuedAudioSize(_sdl_audio_device);
  return (int)((bytes / 2) * 1000u / AUDIO_SAMPLE_RATE);
}

int simuGetAudioHostAudibleMs()
{
  return simuGetAudioBufferedMs();
}
