...
/** True after firmware power-off completes — host should close the App. */
...
// Backlight: 0..BACKLIGHT_LEVEL_MAX (typically 100), honouring EdgeTX on/off state.
uint8_t WASM_EXPORT(simuGetBacklightLevel)();

// Speaker volume: 0..VOLUME_LEVEL_MAX. Set from host when Android volume keys change.
void WASM_EXPORT(simuSetSpeakerVolume)(uint8_t level);
uint8_t WASM_EXPORT(simuGetVolumeMax)();
...
// Stick layout Mode 1..4 stored as 0..3 in g_eeGeneral.stickMode.
...
// Apply full ModelData / RadioData blobs from bridge (sizes must match CHKSIZE).
...
// Value-based trim setting: idx is physical trim index (no stickMode remap).
...
// True while a standalone tool script or fullscreen Lua widget is active.
bool WASM_EXPORT(simuIsLuaScriptActive)();
...
// Milliseconds of PCM still waiting to play on the host. Used by
// audioConsumeCurrentBuffer() as DAC-like backpressure so SIMU does not
// decode an entire WAV in one wakeup (which breaks IS_PLAYING / BG music).
int WASM_IMPORT(simuGetAudioBufferedMs)();

// Estimated ms until host output goes silent (pending queue + device tail).
// Used by isPlaying() on SIMU; not used for producer backpressure.
int WASM_IMPORT(simuGetAudioHostAudibleMs)();

// Drop host-side PCM not yet heard (pending queue + device buffer).
// Called from AudioQueue::flush() on SIMU when firmware clears its queues.
void WASM_IMPORT(simuFlushAudio)();
...
// Android host: phone battery level and charge state for the radio UI.
bool WASM_IMPORT(simuGetHostBattery)(uint16_t* voltage10mV, bool* plugged,
                                     bool* charging);
