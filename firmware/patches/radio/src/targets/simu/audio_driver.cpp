...
// Keep a small head-start on the host (like HW DMA double-buffering), but
// do not dump the whole WAV into the host in one audio task wakeup.
static constexpr int kSimuAudioMaxAheadMs = 120;

void audioConsumeCurrentBuffer()
{
  auto& fifo = audioQueue.buffersFifo;
  while (true) {
    if (simuGetAudioBufferedMs() > kSimuAudioMaxAheadMs) {
      return;
    }
