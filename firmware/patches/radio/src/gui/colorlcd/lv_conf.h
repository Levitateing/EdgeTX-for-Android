...
#if defined(BOOT) || defined(__ANDROID__)
/* Android host: avoid a 16 MB static LVGL pool in the .so BSS */
...
/* Only for high-res / huge CJK glyph sets. Do NOT enable for all RADIO_ANDROID:
 * TX16S F429 uses stock std LZ4 fonts packed for LARGE=0; LARGE=1 mis-parses
 * glyph_dsc (8 vs 16 bytes), blanks BOLD/XL text and can corrupt the heap. */
#ifndef LV_FONT_FMT_TXT_LARGE
#if defined(EDGE_TX_LCD_W) && EDGE_TX_LCD_W >= 1280
