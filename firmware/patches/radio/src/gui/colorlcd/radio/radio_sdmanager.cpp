...
void RadioSdManagerPage::checkEvents()
{
  ...
  if (loadPreview) {
    ...
    if (loadPreview == 0) {
      loading->hide();
      preview->setFile(previewFilename.empty() ? nullptr : previewFilename.c_str());
                                    ...
                                    const char* fullpath)
{
  ...
  const std::string filePath(fullpath ? fullpath : "");
  auto menu = new Menu();
  ...
  menu->addLine(STR_DELETE_FILE, [=]() {
    if (f_unlink(filePath.c_str()) != FR_OK) {
  ...
  if (ext) {
    if (!strcasecmp(ext, SOUNDS_EXT)) {
      menu->addLine(STR_PLAY_FILE, [=]() {
        audioQueue.stopAll();
        audioQueue.playFile(filePath.c_str(), 0, ID_PLAY_FROM_SD_MANAGER);
      });
    }
    ...
    if (!strcasecmp(ext, MULTI_FIRMWARE_EXT)) {
      ...
      if (information.readMultiFirmwareInformation(filePath.c_str()) == nullptr) {
#if defined(INTERNAL_MODULE_MULTI)
        menu->addLine(STR_FLASH_INTERNAL_MULTI, [=]() {
          MultiFirmwareUpdate(filePath.c_str(), INTERNAL_MODULE,
                              MULTI_TYPE_MULTIMODULE);
        });
#endif
        menu->addLine(STR_FLASH_EXTERNAL_MULTI, [=]() {
          MultiFirmwareUpdate(filePath.c_str(), EXTERNAL_MODULE,
                              MULTI_TYPE_MULTIMODULE);
        });
      }
    }
      ...
      menu->addLine(STR_FLASH_EXTERNAL_ELRS, [=]() {
        MultiFirmwareUpdate(filePath.c_str(), EXTERNAL_MODULE, MULTI_TYPE_ELRS);
      });
#endif
      ...
      menu->addLine(STR_VIEW_TEXT, [=]() {
        FIL file;
        if (FR_OK == f_open(&file, filePath.c_str(), FA_OPEN_EXISTING | FA_READ)) {
...
//TODO: Find out why UF2FirmwareUpdate is bricking
      ...
      if (isBootloader(filePath.c_str())) {
        menu->addLine(STR_FLASH_BOOTLOADER,
                      [=]() { BootloaderUpdate(filePath.c_str()); });
      }
#endif
#if defined(HARDWARE_INTERNAL_MODULE) || defined(HARDWARE_EXTERNAL_MODULE)
      ...
      if (mod_desc && mod_desc->set_pwr) {
        menu->addLine(STR_FLASH_EXTERNAL_DEVICE,
                      [=]() { FrSkyFirmwareUpdate(filePath.c_str(), SPORT_MODULE); });
      }
      menu->addLine(STR_FLASH_INTERNAL_MODULE,
                    [=]() { FrSkyFirmwareUpdate(filePath.c_str(), INTERNAL_MODULE); });
      menu->addLine(STR_FLASH_EXTERNAL_MODULE,
                    [=]() { FrSkyFirmwareUpdate(filePath.c_str(), EXTERNAL_MODULE); });
    } else if (!strcasecmp(ext, FRSKY_FIRMWARE_EXT)) {
      FrSkyFirmwareInformation information;
      if (readFrSkyFirmwareInformation(filePath.c_str(), information) ==
          nullptr) {
        ...
        if (information.productFamily == FIRMWARE_FAMILY_RECEIVER) {
          if (isReceiverOTAEnabledFromModule(INTERNAL_MODULE,
            ...
            menu->addLine(STR_FLASH_RECEIVER_BY_INTERNAL_MODULE_OTA, [=]() {
              ...
              dialog->flash(filePath.c_str(), INTERNAL_MODULE);
            });
          ...
          if (isReceiverOTAEnabledFromModule(EXTERNAL_MODULE,
            ...
            menu->addLine(STR_FLASH_RECEIVER_BY_EXTERNAL_MODULE_OTA, [=]() {
              ...
              dialog->flash(filePath.c_str(), EXTERNAL_MODULE);
            });
        ...
        if (information.productFamily == FIRMWARE_FAMILY_BLUETOOTH_CHIP) {
          menu->addLine(STR_FLASH_BLUETOOTH_MODULE, [=]() {
            BluetoothFirmwareUpdate(filePath.c_str());
          });
        }
#endif
#endif  // _NYI_
      ...
      menu->addLine(STR_EXECUTE_FILE, [=]() {
        luaExecStandalone(filePath.c_str());
      });
    }
#endif
  }
