...
void luaExecStandalone(const char * filename)
{
  ...
  PROTECT_LUA() {
    ...
    int loadResult = luaLoadScriptFileToState(lsStandalone, filename, LUA_SCRIPT_LOAD_MODE);
    if (loadResult == SCRIPT_OK) {
      if (lua_pcall(lsStandalone, 0, 1, 0) == LUA_OK && lua_istable(lsStandalone, -1)) {
        ...
        if (runFunction == LUA_REFNIL) {
          lua_pushfstring(lsStandalone, "tool script must return run(): %s", filename);
          extern void luaError(lua_State *L, uint8_t error);
          luaError(lsStandalone, SCRIPT_SYNTAX_ERROR);
        } else {
#if LUA_SCRIPT_LCD_COMPAT
          if (!lvglLayout) {
            luaScriptCompatLcdApplyForTool(filename);
            // scale enable happens when StandaloneLuaWindow creates the buffer
          } else {
            luaScriptCompatLcdReset();
            luaScriptCompatScaleEnable(false);
          }
#endif
      ...
      TRACE("luaLoadFile(%s): load failed (%d)", filename, loadResult);
      if (lua_isstring(lsStandalone, -1)) {
        extern void luaError(lua_State *L, uint8_t error);
        luaError(lsStandalone, loadResult);
      }
    }
  }
  else {
    ...
    useLvgl(useLvgl), initFunction(initFn), runFunction(runFn)
{
  ...
  if (useLvglLayout()) {
    ...
    lcdBuffer->createOffscreenCanvas();
    lcdBuffer->addCanvas(this);

    lcdBuffer->clear();
    lcdBuffer->drawText(LCD_W / 2,
                        LCD_H / 2 - EdgeTxStyles::STD_FONT_HEIGHT,
                        STR_LOADING, FONT(L) | COLOR_THEME_PRIMARY2 | CENTERED);
#if LUA_SCRIPT_LCD_COMPAT
    // Scale script logical coords (e.g. 800x480) onto the physical LCD,
    // letterboxed/centered. Uses system fonts (same as main UI).
    luaScriptCompatScaleApplyLetterbox();
    luaScriptCompatScaleEnable(true);
#endif
...
void StandaloneLuaWindow::setup(bool useLvgl, int initFn, int runFn)
{
  if (_instance != nullptr)
    _instance->shutdownUI();

  _instance = new StandaloneLuaWindow(useLvgl, initFn, runFn);
}

void StandaloneLuaWindow::shutdownUI()
{
  if (_deleted) return;
...
#if LUA_SCRIPT_LCD_COMPAT
static void scaleTouchEventForScriptLcd(LuaEventData* evt)
{
  if (!evt || !IS_TOUCH_EVENT(evt->event)) return;
  if (!luaScriptCompatScaleEnabled) return;

  const float scale = luaScriptCompatScale();
  if (scale <= 0.0f) return;

  evt->touchX = luaScriptCompatUnscaleX(evt->touchX);
  evt->touchY = luaScriptCompatUnscaleY(evt->touchY);

  if (evt->event == EVT_TOUCH_SLIDE) {
    evt->startX = luaScriptCompatUnscaleX(evt->startX);
    evt->startY = luaScriptCompatUnscaleY(evt->startY);
    evt->slideX = luaScriptCompatUnscaleSize(evt->slideX);
    evt->slideY = luaScriptCompatUnscaleSize(evt->slideY);
  }
}
#endif

static void setLuaScriptLcdGlobals()
{
#if LUA_SCRIPT_LCD_COMPAT
  lua_pushinteger(lsStandalone, luaScriptCompatLcdW);
  lua_setglobal(lsStandalone, "LCD_W");
  lua_pushinteger(lsStandalone, luaScriptCompatLcdH);
  lua_setglobal(lsStandalone, "LCD_H");
#endif
}
...
#if LUA_SCRIPT_LCD_COMPAT
        scaleTouchEventForScriptLcd(&evt);
#endif
...
#if LUA_SCRIPT_LCD_COMPAT
              // Fill letterbox bars outside the scaled logical viewport.
              if (luaScriptCompatScaleEnabled && lcdBuffer) {
                const float s = luaScriptCompatScale();
                const coord_t ox = luaScriptCompatOffsetX();
                const coord_t oy = luaScriptCompatOffsetY();
                const coord_t sw = (coord_t)((float)luaScriptCompatLcdW * s);
                const coord_t sh = (coord_t)((float)luaScriptCompatLcdH * s);
                const LcdFlags bar = COLOR2FLAGS(COLOR_THEME_PRIMARY1_INDEX);
                if (oy > 0) {
                  lcdBuffer->drawSolidFilledRect(0, 0, LCD_W, oy, bar);
                  lcdBuffer->drawSolidFilledRect(0, oy + sh, LCD_W, LCD_H - (oy + sh), bar);
                }
                if (ox > 0) {
                  lcdBuffer->drawSolidFilledRect(0, oy, ox, sh, bar);
                  lcdBuffer->drawSolidFilledRect(ox + sw, oy, LCD_W - (ox + sw), sh, bar);
                }
              }
#endif
            }
                                       ...
                                       bool& result)
{
  ...
  // transparent background (physical LCD; POPUP_* already LAYOUT_SCALED)
  lcdBuffer->drawFilledRect(0, 0, LCD_W, LCD_H, SOLID,
                            COLOR_THEME_PRIMARY1, OPACITY(5));

  popupPaint(lcdBuffer, POPUP_X, POPUP_Y, LCD_W - POPUP_X * 2,
             LCD_H - POPUP_Y * 2, text, info);
