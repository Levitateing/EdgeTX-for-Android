...
class MenuWindowContent : public NavWindow
{
 public:
      ...
      NavWindow(parent, rect_t{}, menu_content_create)
  {
    ...
    popupMaxH = LCD_H - PAD_LARGE * 2;
    lv_obj_set_style_max_height(lvobj, popupMaxH, LV_PART_MAIN);
    syncBodyHeight(popupMaxH);
  }

  void syncBodyHeight(coord_t popupHeight)
  {
    lv_obj_update_layout(lvobj);
    coord_t headerH = 0;
    if (!lv_obj_has_flag(header->getLvObj(), LV_OBJ_FLAG_HIDDEN))
      headerH = lv_obj_get_height(header->getLvObj());
    coord_t bodyMax = popupHeight - headerH;
    if (bodyMax < EdgeTxStyles::UI_ELEMENT_HEIGHT)
      bodyMax = EdgeTxStyles::UI_ELEMENT_HEIGHT;
    lv_obj_set_style_max_height(body->getLvObj(), bodyMax, LV_PART_MAIN);
  }
...
{
  if (toolbar) {
    lv_obj_update_layout(toolbar->getLvObj());
    ...
    coord_t maxPopupH = LCD_H - PAD_LARGE * 2;
    coord_t h = min(max(ch, th), maxPopupH);

    toolbar->setHeight(h);
    content->setHeight(h);
    content->syncBodyHeight(h);
