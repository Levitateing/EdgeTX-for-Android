    ...
    cancelHandler(std::move(cancelHandler))
{
  ...
  box->padAll(PAD_TINY);
  box->setFlexLayout(LV_FLEX_FLOW_ROW, EdgeTxStyles::DIALOG_BTN_GAP, LV_PCT(100),
                     LV_SIZE_CONTENT);
  lv_obj_set_flex_align(box->getLvObj(), LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_SPACE_BETWEEN);

  new TextButton(box, rect_t{0, 0, EdgeTxStyles::DIALOG_BTN_WIDTH, 0}, STR_NO,
                 [=]() -> int8_t {
    onCancel();
    return 0;
  });

  new TextButton(box, rect_t{0, 0, EdgeTxStyles::DIALOG_BTN_WIDTH, 0}, STR_YES,
                 [=]() -> int8_t {
    this->deleteLater();
    this->confirmHandler();
    return 0;
    ...
    ModalWindow(false), saveHandler(std::move(_saveHandler))
{
  ...
  new TextButton(box, rect_t{0, 0, EdgeTxStyles::DIALOG_BTN_WIDTH, 0},
                 STR_CANCEL, [=]() {
    deleteLater();
    return 0;
  });

  new TextButton(box, rect_t{0, 0, EdgeTxStyles::DIALOG_BTN_WIDTH, 0}, STR_SAVE,
                 [=]() {
