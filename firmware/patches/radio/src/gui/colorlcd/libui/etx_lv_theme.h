 ...
 *********************/
  ...
  #elif LCD_H >= 480
    // Landscape COLORLCD: scale by height (TX16 ref 1.375 @ H=480).
    // Equivalent to round(x * 1.375 * LCD_H / 480). The +1920 term is the
    // 0.5 rounding bias for denominator 3840 (= 8*480); it must NOT scale
    // with LCD_H (older "+ 4*LCD_H" matched round only when LCD_H==480 and
    // made UI ~1px larger than bitmaps on high-res displays).
    #define LAYOUT_SCALE(x) (((x) * 11 * LCD_H + 1920) / (8 * 480))
    #define LUA_LCD_SCALE (1.375 * (double)LCD_H / 480.0)
  #endif
#endif
...
class EdgeTxStyles
{
 public:
  ...
  static LAYOUT_VAL_SCALED(DIALOG_BTN_WIDTH, 96)
  static LAYOUT_VAL_SCALED(DIALOG_BTN_GAP, 40)
