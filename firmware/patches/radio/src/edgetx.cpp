...
#if defined(__ANDROID__) && defined(SIMU)
#include <android/log.h>
#define ANDROID_BOOT_LOG(step) \
  __android_log_print(ANDROID_LOG_INFO, "EdgeTXFW", "%s", step)
#else
#define ANDROID_BOOT_LOG(step) ((void)0)
#endif
...
#if defined(RADIO_ANDROID) && defined(USB_SERIAL) && !defined(SIMU) && !defined(BOOT)
  {
    static uint8_t android_bridge_started = 0;
    if (!android_bridge_started) {
      androidBridgeInit();
      androidBridgeStart();
      android_bridge_started = 1;
    }
    androidBridgePoll();
  }
#endif
...
#if defined(SIMU) && defined(__ANDROID__)
  // Signal App exit only after storage flush / UI teardown completed.
  extern void simuNotifyHostPowerOffComplete();
  if (shutdown) simuNotifyHostPowerOffComplete();
#endif
}
...
#endif
{
  ANDROID_BOOT_LOG("simuMain: enter");
#if defined(SEMIHOSTING)
  ...
  ANDROID_BOOT_LOG("simuMain: boardInit");
  boardInit();

  ANDROID_BOOT_LOG("simuMain: modulePortInit");
  modulePortInit();
  ANDROID_BOOT_LOG("simuMain: pulsesInit");
  pulsesInit();

#if defined(COLORLCD)
  // Do all lvgl init in case of fatal error on startup
  extern void initLvgl();
  ANDROID_BOOT_LOG("simuMain: initLvgl");
  initLvgl();
  ANDROID_BOOT_LOG("simuMain: initLvgl done");
#endif
  ...
  ANDROID_BOOT_LOG("simuMain: tasksStart");
  tasksStart();
  ANDROID_BOOT_LOG("simuMain: done");
}
...
uint32_t pwrCheck()
{
  ...
  // (enum lives at file scope — see PwrCheckState above)
...
#endif  // !defined(SIMU) || defined(__ANDROID__)
