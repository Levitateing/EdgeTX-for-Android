...
#if defined(SIMU)
    // On hardware, only a few DMA buffers remain after the switch opens, so
    // stop feels near-instant. With SIMU backpressure we only keep ~100ms on
    // the host; if we also stop mixing immediately the track is hard-cut.
    // Finish the current background fragment, but do not start a new one
    // while FUNCTION_BACKGND_MUSIC is inactive (handled in evalFunctions).
    if (!isFunctionActive(FUNCTION_BACKGND_MUSIC_PAUSE) &&
        (isFunctionActive(FUNCTION_BACKGND_MUSIC) ||
         !backgroundContext.isEmpty())) {
#else
    if (isFunctionActive(FUNCTION_BACKGND_MUSIC) &&
        !isFunctionActive(FUNCTION_BACKGND_MUSIC_PAUSE)) {
#endif
...
{
  bool playing = normalContext.hasPromptId(id) ||
#if defined(SIMU)
...
#if defined(SIMU)
  // Host (pending + AudioTrack tail) may still be audible after the firmware
  // context drained. Keep IS_PLAYING true so BG is not restarted early.
  if (!playing && isFunctionActive(FUNCTION_BACKGND_MUSIC) &&
      id == simu_last_bg_id_ && simuGetAudioHostAudibleMs() > 0) {
    playing = true;
  }
#endif
